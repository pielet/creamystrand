...
namespace strandsim
{
	...
	void QuasiNewtonStepper::prepareStep(Scalar dt)
	{
		...
		m_strand.setFutureDegreesOfFreedom(m_strand.getCurrentDegreesOfFreedom() + m_velocities * m_dt);
		m_timer.restart();
		m_dynamics.computeFutureJacobian(false, m_params.m_energyWithTwist, m_params.m_energyWithBend);
		m_bendTwistHessian = m_strand.getFutureTotalJacobian();
		m_bendTwistHessian *= m_dt * m_dt;
		m_dynamics.addMassMatrixTo(m_bendTwistHessian);
		m_timing.hessian += m_timer.elapsed();
	}
	...
	bool QuasiNewtonStepper::performOneIteration()
	{
		...
		// stretching hessian
		m_timer.restart();
		m_dynamics.computeFutureJacobian(m_params.m_energyWithStretch, false, false);
		JacobianMatrixType stretching_hessian = m_strand.getFutureTotalJacobian();
		stretching_hessian *= m_dt * m_dt;
		m_bendTwistHessian += stretching_hessian;
		m_dynamics.getScriptingController()->fixLHS(m_bendTwistHessian);
		m_timing.hessian += m_timer.elapsed();

		m_timer.restart();
		m_directSolver.store(m_bendTwistHessian);

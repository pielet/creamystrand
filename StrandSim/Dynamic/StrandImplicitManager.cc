...
namespace strandsim
{
		...
		, m_substep_callback(sub_callback)
	{
		...
		m_selfCollisionTimes.resize(m_strands.size());
		m_externalCollisionTimes.resize(m_strands.size());
...
#pragma omp critical(mutualContact)
				...
				if (VertexFaceCollision* vfCollision = dynamic_cast<VertexFaceCollision*>(ctCollision)) // Assignment =
				{
					...
					collision.objects.second.freeVel = vfCollision->meshVelocity(dt);
				...
				else if (EdgeFaceCollision* efCollision = dynamic_cast<EdgeFaceCollision*>(ctCollision)) // Assignment =
				{
					...
					collision.objects.second.freeVel = efCollision->meshVelocity(dt);
			...
			for (int itIdx = 0; itIdx < nColObj; ++itIdx)
			{
				...
				for (auto second = first->second.begin(); second != first->second.end(); ++second)
				{
						...
						++collision)
					{
						...
						mutualContact.objects.first.direction = (sP->getVertex(iP + 1) - sP->getVertex(iP)).normalized();
						...
						mutualContact.objects.second.direction = (sQ->getVertex(iQ + 1) - sP->getVertex(iQ)).normalized();
						...
						if (acceptFirst && acceptSecond) {
							...
							if (std::find(m_mutualContacts.begin(), m_mutualContacts.end(), mutualContact) == m_mutualContacts.end()) {
#pragma omp critical(mutualContact)
	...
	*/
	...
	void StrandImplicitManager::updateCollisionTimes(const ProximityCollisions& contacts)
	{
		for (int i = 0; i < contacts.size(); ++i)
		{
			...
			if (s2 == -1) {
				++m_externalCollisionTimes[s1][v1];
			}
			else {
				++m_selfCollisionTimes[s1][v1];
				++m_selfCollisionTimes[s2][v2];
			}
		}
	}
...
#pragma omp parallel for 
			...
			if (m_params.m_innerIterations > 0 && m_statTotalContacts > 0) {
				timer.restart();
				for (int j = 0; j < m_params.m_innerIterations; ++j) {
		...
		if (m_params.m_postProcessIterations > 0 && m_statTotalContacts > 0) {
			timer.restart();
			for (int j = 0; j < m_params.m_postProcessIterations; ++j) {
				step_solveCollisions(false);
			}
			timings.solve += timer.elapsed();
		}
...
#pragma omp parallel for
	...
	void StrandImplicitManager::step_prepareCollision()
	{
		...
		for (int i = 0; i < m_strands.size(); ++i)
		{
			...
			m_selfCollisionTimes[i].clear();
			m_externalCollisionTimes[i].clear();
		}
...
#pragma omp parallel for 
		for (int i = 0; i < m_mutualContacts.size(); ++i) {
			...
			collisionWarmStarting(m_mutualContacts[i], true);
...
#pragma omp parallel for
	...
	void StrandImplicitManager::collisionWarmStarting(ProximityCollision& collision, bool isMutual)
	{
		const ProximityCollision* oldCollision = m_collisionDatabase.find(collision);

		if (oldCollision)
		{
			const Vec3x force = parallelTransport(oldCollision->force, oldCollision->normal, collision.normal);
			collision.force = force * m_params.m_relaxationFactor;
			...
			if (isMutual) {
				...
				m_steppers[s1]->accumulateDeltaCollisionImpulse(v1, (1 - alpha_1) * collision.force);
				m_steppers[s1]->accumulateDeltaCollisionImpulse(v1 + 1, alpha_1 * collision.force);
				m_steppers[s2]->accumulateDeltaCollisionImpulse(v2, -(1 - alpha_2) * collision.force);
				m_steppers[s2]->accumulateDeltaCollisionImpulse(v2 + 1, -alpha_2 * collision.force);
			}
			else {
				...
				m_steppers[sid]->accumulateDeltaCollisionImpulse(vid, collision.force);
			}
		}
	}
	...
	{
		std::vector<std::map<unsigned, unsigned> > tmp_selfCollisionTime(m_strands.size());
		std::vector<std::map<unsigned, unsigned> > tmp_externalCollisionTime(m_strands.size());

		//for (int i = 0; i < m_steppers.size(); ++i)
		//	m_steppers[i]->clearCollisionVelocity();

		//	Scalar alpha = 0;
		//	int n_col_strand = 0;
		//	for (int i = 0; i < m_strands.size(); ++i) {
		//		if (m_selfCollisionTimes[i].size() > 0) {
		//			alpha += m_steppers[i]->stepSize();
		...
		Vec3x mutual_delta_r = Vec3x::Zero();
		...
		{ 
			...
			Scalar cr1 = m_strands[s1]->collisionParameters().responseRadius();
			Scalar cr2 = m_strands[s2]->collisionParameters().responseRadius();
			Scalar offset = collision.distance - cr1 - cr2;

			Scalar m1 = m_strands[s1]->getEdgeMass(v1) / m_selfCollisionTimes[s1][v1];
			Scalar m2 = m_strands[s2]->getEdgeMass(v2) / m_selfCollisionTimes[s2][v2];
			...
			//Vec3x r = solveOneCollision(K, cv2 - cv1 - offset / m_dt * collision.normal, collision.normal, collision.mu);

			//accumulateImpulse(s1, v1, alpha_1, r, commitVelocities);
			//accumulateImpulse(s2, v2, alpha_2, -r, commitVelocities);

			const Vec3x r = solveOneCollision(cv2 - cv1 - offset / m_dt * collision.normal, m1, m2, collision.normal, collision.mu);

			m_steppers[s1]->accumulateDeltaCollisionImpulse(v1, (1 - alpha_1) * r);
			m_steppers[s1]->accumulateDeltaCollisionImpulse(v1 + 1, alpha_1 * r);
			m_steppers[s2]->accumulateDeltaCollisionImpulse(v2, -(1 - alpha_2) * r);
			m_steppers[s2]->accumulateDeltaCollisionImpulse(v2 + 1, -alpha_2 * r); 

			ContactStream(g_log, "") << "delta_r: " << r; 
			...
			if (r.norm() < m_params.m_collisionSolverTolerace) {
				collision.solved = true;
			}
			else {
				++tmp_selfCollisionTime[s1][v1];
				++tmp_selfCollisionTime[s2][v2];
			}
		}

		Vec3x external_delta_r = Vec3x::Zero();
		for (int i = 0; i < m_strands.size(); ++i)
		{
			for (ProximityCollision& collision : m_externalContacts[i])
			{
				...
				if (isSmall(alpha))	// vertex-face
				{
					...
					m_steppers[sid]->accumulateDeltaCollisionImpulse(vid, r);
					...
					if (r.norm() < m_params.m_collisionSolverTolerace) {
						...
						++tmp_externalCollisionTime[sid][vid];
					}
				}
			}
		}
		...
		for (int i = 0; i < m_strands.size(); ++i)
		{
			m_selfCollisionTimes[i].swap(tmp_selfCollisionTime[i]);
			m_externalCollisionTimes[i].swap(tmp_externalCollisionTime[i]);
		}
		...
		if (m_params.m_statGathering == 2) {
			for (int i = 0; i < m_strands.size(); ++i) {
				for (int j = 0; j < m_strands[i]->getNumVertices(); ++j) {
					Vec3x impulse = m_steppers[i]->totalCollisionImpulses().segment<3>(4 * j);
					if (impulse.norm() > 0)
...
//		Scalar mutual_err = 0;
//#pragma omp parallel for reduction(+:mutual_err)
//		for (int i = 0; i < m_mutualContacts.size(); ++i) {
...
//			Scalar offset = col.distance - 2 * m_strands[s1]->collisionParameters().responseRadius();
//
...
//		}
//		InfoStream(g_log, "mutual") << "FB value: " << mutual_err / m_mutualContacts.size() << " delta_r: " << mutual_delta_r.norm() / m_mutualContacts.size();
//		
//		Scalar external_err = 0;
//#pragma omp parallel for reduction(+:external_err)
...
//				Scalar offset = col.distance - 1e-3;
//
//				Vec3x v_ref = col.transformationMatrix.transpose() * (m_steppers[sid]->getVelocity(vid) - col.objects.second.freeVel);
//				v_ref(0) += offset / m_dt;
//				const Vec3x r = col.transformationMatrix.transpose() * col.force;
//
//				Vec3x s;
...
//			}
//		}
//		int n_external_col = m_statTotalContacts - m_mutualContacts.size();
//		InfoStream(g_log, "external") << "FB value: " << external_err / n_external_col << " delta_r: " << external_delta_r.norm() / n_external_col;
}
	...
	Vec3x StrandImplicitManager::solveOneCollision(const Vec3x& reletive_vel, Scalar m1, Scalar m2, const Vec3x& normal, Scalar mu)
	{
		const Vec3x r = (m1 * m2 / (m1 + m2)) * reletive_vel;
		Scalar r_n = r.dot(normal);
	...
	void StrandImplicitManager::accumulateImpulse(int sid, int vid, Scalar alpha, const Vec3x r, bool commitVelocity)
	{
		...
		m_steppers[sid]->accumulateDeltaCollisionImpulse(vid, delta_v1 * m1);
		m_steppers[sid]->accumulateDeltaCollisionImpulse(vid + 1, delta_v2 * m2);
	}
	...
	void StrandImplicitManager::step_postCollision()
	{
		...
		// update database
		if (!m_params.m_simulationManager_limitedMemory) {
			for (const auto cp : colPointers) {
				m_collisionDatabase.insert(*cp);
			}
		}
		...
		// print impulse norm in collision node
		if (m_params.m_statGathering == 2) {
			for (int i = 0; i < m_strands.size(); ++i) {
				for (int j = 0; j < m_strands[i]->getNumVertices(); ++j) {
					Vec3x impulse = m_steppers[i]->totalCollisionImpulses().segment<3>(4 * j);
